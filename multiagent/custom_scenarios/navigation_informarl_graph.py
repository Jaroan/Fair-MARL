...
class Scenario(BaseScenario):
	...
	def info_callback(self, agent: Agent, world: World) -> Tuple:
		...
		agent_info = {
			...
			'Distance_mean': world.dist_left_to_goal[agent.id], 
			'Distance_variance': world.dist_left_to_goal[agent.id],
			'Mean_by_variance': world.dist_left_to_goal[agent.id],
			'Dists_traveled': world.dist_left_to_goal[agent.id],
			'Time_taken': world.dist_left_to_goal[agent.id],
		}
		if self.max_speed is not None:
