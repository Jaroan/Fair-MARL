...
class Scenario(BaseScenario):
	...
	# def fairness_appended_reward(self, agent:Agent, world:World) -> float:
		...
		# print(agent.id,"matched",self.optimal_match_index[agent.id])
		## optimal 1:1 agent-landmark pairing (bipartite matching algorithm)
		world.dists = np.array([[np.linalg.norm(a.state.p_pos - l.state.p_pos) for l in world.landmarks]
								   for a in world.agents])
		...
		if dist_to_goal < self.min_dist_thresh:
			rew += self.goal_rew
			# print(agent.id,"goal",self.optimal_match_index[agent.id])
		else:
			rew -= dist_to_goal
	...
	def observation(self, agent:Agent, world:World) -> arr:
		...
		# ## print("obs", len(entity_pos), entity_pos)
		# ## print("concat",np.concatenate([agent.state.p_vel, agent.state.p_pos] + entity_pos).shape)
		# ## print(np.concatenate([agent.state.p_vel, agent.state.p_pos] + entity_pos))
	...
	def _get_entity_feat_relative(self, agent:Agent, entity:Entity, world:World, fairness_param: np.ndarray) -> arr:
		...
		if 'agent' in entity.name:
			...
			# # print("rel-goal", len(rel_goal_pos),rel_goal_pos)
			# # print("rel_goal_pos",rel_goal_pos)

			## rel_goal_pos = np.array(rel_goal_pos)
			entity_type = entity_mapping['agent']
		elif 'landmark' in entity.name:
			...
			## print("wall hstack" ,np.hstack([rel_vel, rel_pos,rel_pos,wall_o_corner,wall_d_corner, entity_type]).shape)
			## print("wahnot",np.concatenate([rel_vel, rel_pos,rel_pos,rel_pos, entity_type]))
			...
			raise ValueError(f'{entity.name} not supported')
		# print("hstack" ,np.hstack([rel_vel, rel_pos, rel_goal_pos,rel_pos,rel_pos, entity_type]).shape, np.hstack([rel_vel, rel_pos, rel_goal_pos,rel_pos,rel_pos, entity_type]))
		# print("hnot",np.hstack([rel_vel, rel_pos,rel_pos,rel_pos, entity_type]))
