...
class Scenario(BaseScenario):
	...
	def random_scenario(self, world):
		...
		# uniform_pos = np.linspace(-0.8*self.world_size/2, 0.8*self.world_size/2, self.num_obstacles)

		# for i, obstacle in enumerate(world.obstacles):
		# 	obstacle.state.p_pos = np.array([uniform_pos[i], -uniform_pos[i]+ 0.4])
		# 	obstacle.state.p_vel = np.zeros(world.dim_p)
		# print("all obstacles pos", [obstacle.state.p_pos for obstacle in world.obstacles])
	...
	def info_callback(self, agent:Agent, world:World) -> Tuple:
		...
		# print("______INFO CALLBACK",world.dist_traveled_mean,world.dist_traveled_stddev, world.dists_to_goal)
		...
		# print("agent", agent.id,"time taken",world.times_required[agent.id],"mean",world.time_taken_mean,"stddev", world.time_taken_stddev)
		agent_info = {
	...
	# def fairness_appended_reward(self, agent:Agent, world:World) -> float:
		...
		if world.dists_to_goal[agent.id] == -1:
			...
			# print("MEAN1","{:.3f}".format(mean_dist),"std_dev","{:.3f}".format(std_dev_dist))
		else:
			fairness_param = world.dist_traveled_mean/(world.dist_traveled_stddev+0.0001)
			# print("MEAN2","{:.3f}".format(world.dist_traveled_mean),"std_dev",world.dist_traveled_stddev)
		...
		if len(agent_pos) > 1:
			costs = dist.cdist(agent_pos, self.landmark_poses_updated)
		...
		# self.landmark_poses_occupied[mask] = 1.0
			...
			self.goal_match_index = np.where(x==1)[1]
		print("agents left",len(agent_pos),"goals left",len(self.landmark_poses_updated))

		print("goal_match_index",self.goal_match_index)
		if self.goal_match_index.size == 0 or self.goal_match_index.shape != (self.num_agents,):
			self.goal_match_index = np.arange(len(agent_pos))
		if agent.status == False:
			print("agent_id_updated",self.agent_id_updated)
			agent_id_now = np.where( self.agent_id_updated==agent.id)[0]
			print("AGENT",agent.id,"agent_id_now",agent_id_now,"self.goal_match_index[agent_id_now]",self.goal_match_index[agent_id_now])
			dist_to_fair_goal = np.linalg.norm(agent.state.p_pos - self.landmark_poses_updated[self.goal_match_index[agent_id_now]])
		else:
			dist_to_fair_goal = 0.0
		# dist_to_fair_goal = np.linalg.norm(agent.state.p_pos - self.landmark_poses_updated[self.goal_match_index[agent_id_now]])

		print("AGENT",agent.id,"dist_to_fair_goal","{:.3f}".format(dist_to_fair_goal))
		if dist_to_fair_goal < self.min_dist_thresh:
			...
			if agent.status ==False:
				...
				rew += self.goal_rew
				agent_id_now = np.where( self.agent_id_updated==agent.id)[0]
				self.landmark_poses_updated = np.delete(self.landmark_poses_updated, self.goal_match_index[agent_id_now], axis=0)
				self.agent_id_updated = np.delete(self.agent_id_updated,agent_id_now)
		else:
			# # print("AGENT",agent.id,"dist_to_fair_goal","{:.3f}".format(dist_to_fair_goal))
		...
		if agent.collide:
			for a in world.agents:
				...
				if self.is_collision(a, agent):
					...
					# print("collision_rew",self.collision_rew)
		...
		# if fair_rew > self.fair_rew:
		# 	fair_rew = self.fair_rew
		# print("fair","{:.3f}".format(fairness_param),"fair_rew","{:.3f}".format(fair_rew))
			
		
		rew += fair_rew 
		# print("rew","{:.3f}".format(rew),"clipped","{:.3f}".format(np.clip(rew, -2*self.collision_rew, self.goal_rew+self.fair_rew)))
	...
	def observation(self, agent:Agent, world:World) -> arr:
		...
		if min_dist < self.min_obs_dist:
			...
			if len(unoccupied_goals) > 0:
				...
				# second_index = np.where((self.landmark_poses == second_closest_goal).all(axis=1))[0]
				# if top_two_indices[1] == first_index:
				# 	print("First", first_index,"Second", top_two_indices[1], "second",second_index)
				# 	print(agent.id,"FIRST and SECOND is same HERE TOO")
				## check if the goal is occupied
				goal_occupied = np.array([self.landmark_poses_occupied[unoccupied_goals_indices[min_dist_goal]]])
				goal_history = self.goal_history[unoccupied_goals_indices[min_dist_goal]]
				# print("ob2 agents", agent.id,"goal_occupied flag",goal_occupied)
				# print("agents", agent.id,"goal",agents_goal)
		...
		# scaled_input = fairness_param
		# tanh_output = np.array([np.tanh(scaled_input-self.zeroshift)])
		...
		# return np.concatenate((agent.state.p_vel, agent.state.p_pos, goal_pos,goal_occupied,goal_history, rel_second_closest_goal,second_closest_goal_occupied,tanh_output) )# + goal_occupied+ goal_history)
	...
	def graph_observation(self, agent:Agent, world:World) -> Tuple[arr, arr]:
		...
		# fairness_param = np.array([fairness_param])
		# scaled_input = fairness_param
		# fairness_param = np.array(np.tanh(scaled_input-self.zeroshift))
		fairness_param = 0.0
