...
class Scenario(BaseScenario):
	def make_world(self, args:argparse.Namespace) -> World:
		...
		self.num_agents_per_team = args.num_agents_per_team
		...
		for team_id in range(self.num_teams):
			team = [Agent() for _ in range(self.num_agents_per_team)]
			world.agents.append(team)
	...
	def reset_world(self, world:World) -> None:
		...
		for team_id, team in enumerate(world.agents):
			for i, agent in enumerate(team):
				print("team",team_id)
				if i%3 == 0 and team_id ==0:
					agent.color = np.array([0.15, 0.65, 0.85])
				elif i%3 == 1:
					agent.color = np.array([0.85, 0.65, 0.15])
				elif i%3 == 2:
					agent.color = np.array([0.15, 0.85, 0.35])
				elif i%3 == 1 and team_id == 1:
	...
	def random_scenario(self, world):
		...
		while True:
			...
			team = num_agents_added//self.num_agents_per_team
			current_agent_id = num_agents_added%self.num_agents_per_team

			agent_size = world.agents[team][current_agent_id].size
			...
			if not obs_collision and not agent_collision:
				world.agents[team][current_agent_id].state.p_pos = line_pos
				world.agents[team][current_agent_id].state.p_vel = np.zeros(world.dim_p)
				world.agents[team][current_agent_id].state.c = np.zeros(world.dim_c)
				agents_added.append(world.agents[team][current_agent_id])
				num_agents_added += 1
		...
		world.dists = np.array([[np.linalg.norm(a.state.p_pos - l.state.p_pos) for l in world.landmarks]
						   for team in world.agents for a in team])
	...
	def info_callback(self, agent:Agent, world:World) -> Tuple:
		...
		# agent_dist_traveled = np.zeros(self.num_agents)
		# agent_time_taken = np.zeros(self.num_agents)
		...
		agent_info = {
			...
			'Dists_traveled': world.agent_dist_traveled[agent.id],
	...
	def graph_observation(self, agent:Agent, world:World) -> Tuple[arr, arr]:
		...
		"""
		# print("world.entities",world.entities)
	...
	def _get_entity_feat_relative(self, agent:Agent, entity:Entity, world:World) -> arr:
		...
		if 'agent' in entity.name:
			if agent.team_id == entity.team_id:
...
if __name__ == "__main__":
	...
	class Args:
		def __init__(self):
			...
			self.num_agents_per_team:int=3
