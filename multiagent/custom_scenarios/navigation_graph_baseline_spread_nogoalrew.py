...
class Scenario(BaseScenario):
	...
	# def fairness_appended_reward(self, agent:Agent, world:World) -> float:
		...
		landmark_positions = np.array([l.state.p_pos for l in world.landmarks])

		# Calculate distances using NumPy broadcasting
		dists = np.sqrt(np.sum(np.square(agent_positions[:, np.newaxis] - landmark_positions), axis=2))

		# Subtract the minimum distance from each agent's reward
		rew -= np.min(dists, axis=1)
	...
	def observation(self, agent:Agent, world:World) -> arr:
		...
		return np.concatenate([agent.state.p_vel, agent.state.p_pos] + entity_pos + other_pos) ##+ goal_pos)
