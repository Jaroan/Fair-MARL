...
class Scenario(BaseScenario):
	def make_world(self, args:argparse.Namespace) -> World:
		...
		# add landmarks (goals)
		world.landmarks_c= [Landmark() for i in range(int(self.num_landmarks/2))]
		world.landmarks_d= [Landmark() for i in range(int(self.num_landmarks/2))]
		# world.scripted_agents_goals = [Landmark() for i in range(num_scripted_agents_goals)]
		for i, landmark in enumerate(world.landmarks_c):
			landmark.id = i
			...
			global_id += 1
		for i, landmark in enumerate(world.landmarks_d):
			landmark.id = i
			landmark.name = f'landmark {i}'
			landmark.collide = False
			landmark.movable = False
			landmark.global_id = global_id
			global_id += 1
		world.landmarks = world.landmarks_c + world.landmarks_d
		# add obstacles
	...
	def reset_world(self, world:World) -> None:
		...
		for i, landmark in enumerate(world.landmarks_d):
			...
			# 	landmark.color = np.array([0.85, 0.35, 0.35])
			# else:
			landmark.color = np.array([0.85, 0.35, 0.25])
	...
	def random_scenario(self, world):
		...
		while True:
			...
			random_pos = np.random.uniform(-self.world_size/2, 
											self.world_size/2, 
											world.dim_p)
			# random_pos = np.array([num_agents_added/10+0.050, num_agents_added/10+0.00])
			# print("Agent pos",random_pos)
			line_pos = random_pos
		...
		while True:
			if num_goals_added == self.num_landmarks/2:
				break
			...
			random_pos = 0.5 * np.random.uniform(-self.world_size/2, 
												self.world_size/2, 
												world.dim_p)
			# random_pos = np.array([num_goals_added/10-0.050, num_goals_added/10+0.0])
			# print("goal pos",random_pos)
			line_pos = random_pos




			goal_size = world.landmarks_c[num_goals_added].size
			obs_collision = self.is_obstacle_collision(line_pos, goal_size, world)
			landmark_collision = self.is_landmark_collision(line_pos, 
												goal_size, 
												world.landmarks_c[:num_goals_added])
			if not landmark_collision and not obs_collision:
			# if not landmark_collision:
				world.landmarks_c[num_goals_added].state.p_pos = line_pos
				world.landmarks_c[num_goals_added].state.p_vel = np.zeros(world.dim_p)
				num_goals_added += 1
		goal_pos_c = [goal.state.p_pos for goal in world.landmarks_c]
		num_goals_added = 0
		...
		while True:
			...
			goal_size = world.landmarks_d[num_goals_added].size
			obs_collision = self.is_obstacle_collision(line_pos, goal_size, world)
			landmark_collision = self.is_landmark_collision(line_pos, 
												goal_size, 
												world.landmarks_d[:num_goals_added])
			if not landmark_collision and not obs_collision:
			# if not landmark_collision:
				world.landmarks_d[num_goals_added].state.p_pos = line_pos
				world.landmarks_d[num_goals_added].state.p_vel = np.zeros(world.dim_p)
				num_goals_added += 1
		goal_pos_d = [goal.state.p_pos for goal in world.landmarks_d]
		# set wall at end of world positions 
		# and also check collisions with already placed goals
		# num_goals_added = 0
		# goals_added = []
		# merge world.landmarks_c and world.landmarks_d into self.expected_poses
		self.expected_poses = np.concatenate((goal_pos_c, goal_pos_d), axis=0)

		self.expected_poses_occupied = np.zeros(self.num_landmarks)
	...
	def info_callback(self, agent:Agent, world:World) -> Tuple:
		...
		if np.min(world.dists) < self.min_dist_thresh and (world.times_required[agent.id] == -1):
		...
		if world.times_required[agent.id] == -1:
			# print("agent", agent.id, "not reached goal yet")
			world.dists_to_goal[agent.id] = agent.state.p_dist
		...
		# 			# print(a.id,"agent_dist_traveled",self.agent_dist_traveled)
		# 			continue
	...
	def min_time(self, agent:Agent, world:World) -> float:
		...
		dist = np.sqrt(np.sum(np.square(agent.state.p_pos - 
										world.landmarks_c[agent.id].state.p_pos)))
	...
	def reward(self, agent, world):
		...
		if agent.id ==0:
			# agents_goal = world.get_entity(entity_type='landmark', id=world.landmarks_c[agent.id])
			dist_to_goal = np.sqrt(np.sum(np.square(agent.state.p_pos - 
												world.landmarks_c[agent.id].state.p_pos)))
			if dist_to_goal < self.min_dist_thresh+0.05:
				rew += self.goal_rew
				cooperate += 1
			else:
				rew -= dist_to_goal
			# print("dummy cooperate",rew)
		else:
			# print("reward expected_poses",self.expected_poses)
			self.dists_c = np.array(
				[
					[np.linalg.norm(a.state.p_pos - landmark.state.p_pos) for landmark in world.landmarks_c]
					for a in world.agents
				]
			)
			self.dists_d = np.array(
				[
					[np.linalg.norm(a.state.p_pos - landmark.state.p_pos) for landmark in world.landmarks_d]
					for a in world.agents
				]
			)
			...
			mask_c = self.dists_c < self.min_dist_thresh
			self.expected_poses_occupied_c = np.any(mask_c, axis=0).astype(int)
			mask_d = self.dists_d < self.min_dist_thresh
			self.expected_poses_occupied_d = np.any(mask_d, axis=0).astype(int)
			...
			self.delta_dists_c = self._bipartite_min_dists(self.dists_c)
			self.delta_dists_d = self._bipartite_min_dists(self.dists_d)
			# print("delta_dists",self.delta_dists)
			world.dists = self.delta_dists_c

			# print("self.goal_rew",self.goal_rew,"self.delta_dists",self.delta_dists[agent.id])
			## non shared rewards
			if self.delta_dists_c[agent.id] < self.min_dist_thresh:
				# print("goal_rew",self.goal_rew)
				rew += self.goal_rew # cooperating agent gets less reward
				cooperate += 1
				print("cooperate",cooperate,self.delta_dists_c[agent.id])
			elif self.delta_dists_d[agent.id] < self.min_dist_thresh:
				# print("goal_rew",self.goal_rew)
				rew += self.goal_rew # defecting agent gets more reward
				defect += 1
				print("defect",defect)
			else:
				# print("delta_dists",self.delta_dists[agent.id])
				rew -= np.minimum(self.delta_dists_c[agent.id], self.delta_dists_d[agent.id])
		# if agent.collide:
		# 	for a in world.agents:
		...
		rew = np.clip(rew, -2*self.collision_rew, self.goal_rew+2*self.collision_rew)
		return rew, cooperate, defect
	...
	def observation(self, agent:Agent, world:World) -> arr:
		...
		if agent.id ==0:
			goal_pos = []
			# agents_goal = world.get_entity('landmark', self.goal_match_index[agent.id])
			goal_pos.append(world.landmarks_c[agent.id].state.p_pos - agent.state.p_pos)
