...
class Scenario(BaseScenario):
	...
	def random_scenario(self, world):
		...
		goal_radius = 0.7 * self.world_size/2
		...
		while True:
			...
			# print("obs_collision",obs_collision,"landmark_collision",landmark_collision)
	...
	def info_callback(self, agent:Agent, world:World) -> Tuple:
		...
		goal = world.get_entity('landmark',self.goal_match_index[agent.id])
	...
	# def fairness_appended_reward(self, agent:Agent, world:World) -> float:
		...
		rew = 0
		# #-----------------coverage reward--------------------
		# # Agents are rewarded based on distance to any of it's its landmark, 
		...
		# # print(world.dists, "min dist",self.min_dists)

		# # goals_pos = np.zeros((self.num_agents,2))
		# # for ag in range(self.num_agents):
		# 	## print("ag",ag)
		# 	# goals_pos[ag] = world.get_entity(entity_type='landmark', id=ag).state.p_pos
		# # print("goals pos",goals_pos, "ag pos",agent.state.p_pos)
		# # print(agent.state.p_pos - goals_pos,np.sum(np.square(agent.state.p_pos - goals_pos), axis=1))
		# # agents_goal = world.get_entity(entity_type='landmark', id=agent.id)
		# # dist_to_goal = np.sqrt(np.sum(np.square(agent.state.p_pos - 
		# # 										goals_pos),axis=1))
		# # print("dist to goals",dist_to_goal,self.min_dist_thresh)
		# # if np.any(dist_to_goal) < self.min_dist_thresh:
		# if self.min_dists[agent.id] < self.min_dist_thresh:
		# 	rew += self.goal_rew
		# else:
		# 	rew -= self.min_dists[agent.id]

		#----------------goal assigned reward--------------------
	
		# goal_pos = self.collect_goal_info(world)
		## print("goal Pos are",goal_pos.shape, goal_pos.flatten())
		# mean_dist,  std_dev_dist, agent_pos = self.collect_dist(world) # TODO: current func is a bad way to calc mean, std dev

		# costs = dist.cdist(agent_pos, goal_pos)
		# x, objs = solve_fair_assignment(costs)
		# goal_match_index = np.where(x==1)[0]

		# if goal_match_index.size == 0 or goal_match_index.shape != (self.num_agents,):
		# 	print("goal_match_index",goal_match_index)
		# 	goal_match_index = np.arange(self.num_agents)
		...
		# else:
		if world.dists_to_goal[agent.id] == -1:
			mean_dist,  std_dev_dist, agent_pos = self.collect_dist(world)
			...
			# print("world.mean_dist",world.dist_traveled_mean,"world.std_dev_dist",world.dist_traveled_stddev)

		agents_goal = world.get_entity(entity_type='landmark', id=self.goal_match_index[agent.id])
		dist_to_goal = np.sqrt(np.sum(np.square(agent.state.p_pos - 
		...
		world.dists = np.array([[np.linalg.norm(a.state.p_pos - l.state.p_pos) for l in world.landmarks]
								   for a in world.agents])
		# self.min_dists = self._bipartite_min_dists(world.dists)
		# print(np.argmin(world.dists[agent.id]),world.dists[agent.id])

		if np.min(world.dists[agent.id]) < self.min_dist_thresh:
			# self.goal_reached[agent.id,np.argmin(world.dists[agent.id])] +=1
			if np.argmin(world.dists[agent.id]) == self.goal_match_index[agent.id]:
				self.goal_reached[agent.id] += 1
				# print(agent.id,"matched",np.argmin(world.dists[agent.id]))
			else:
				self.wrong_goal_reached[agent.id] += 1
				# print(agent.id,"reached",np.argmin(world.dists[agent.id]))

			# print(agent.id, self.goal_reached[agent.id], "goal reached")
			# print(np.argmin(world.dists[agent.id]),np.min(world.dists[agent.id]), "min dist",self.min_dists)
		...
		# print("mean", mean_dist, "std_dev_dist", std_dev_dist, "fairness", mean_dist/(std_dev_dist+0.001))
	...
	def _get_entity_feat_relative(self, agent:Agent, entity:Entity, world:World) -> arr:
		...
		if 'agent' in entity.name:
			...
			return np.hstack([rel_vel, rel_pos, rel_goal_pos,wall_o_corner,wall_d_corner, entity_type])
			# return np.hstack([rel_vel, rel_pos, rel_goal_pos, entity_type,wall_o_corner])#,wall_d_corner])

		else:
			raise ValueError(f'{entity.name} not supported')

		return np.hstack([rel_vel, rel_pos, rel_goal_pos,rel_pos,rel_pos, entity_type])
