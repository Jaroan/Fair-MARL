...
class Scenario(BaseScenario):
	...
	def observation(self, agent:Agent, world:World) -> arr:
		...
		if np.min(world.dists) < self.min_dist_thresh:
			...
			# print("obs1 agent", agent.id, "goal_pos",np.min(world.dists),chosen_goal, "landmark_poses_occupied",self.landmark_poses_occupied[chosen_goal])
		else:
			...
			if len(unoccupied_goals) > 0:
				...
				# print("obs2 agent", agent.id, "goal_pos",np.min(world.dists), min_dist_goal,"landmark_poses_occupied",goal_occupied)
			else:
	...
	def _get_entity_feat_relative(self, agent:Agent, entity:Entity, world:World, fairness_param: np.ndarray) -> arr:
		...
		if 'agent' in entity.name:
			...
			if np.min(world.dists) < self.min_dist_thresh:
				...
				self.landmark_poses_occupied[chosen_goal] = 1
				# print("gr1 agent", entity.id, "goal_pos",np.min(world.dists),chosen_goal, "landmark_poses_occupied",self.landmark_poses_occupied[chosen_goal])
				goal_occupied = np.array([self.landmark_poses_occupied[chosen_goal]])
			else:
				unoccupied_goals = self.landmark_poses[self.landmark_poses_occupied==0]
				...
				if len(unoccupied_goals) > 0:
					...
					## check if the goal is occupied
					goal_occupied = np.array([self.landmark_poses_occupied[unoccupied_goals_indices[min_dist_goal]]])
					# print("gr2 agent", entity.id, "goal_pos",np.min(world.dists), min_dist_goal,"landmark_poses_occupied",goal_occupied)
