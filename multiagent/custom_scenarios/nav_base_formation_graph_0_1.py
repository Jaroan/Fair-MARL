...
class Scenario(BaseScenario):
	def make_world(self, args:argparse.Namespace) -> World:
		...
		# print("world.dists_to_goal",world.dists_to_goal)
	...
	def info_callback(self, agent:Agent, world:World) -> Tuple:
		...
		# print("Hi agent", agent.id, world.dists_to_goal, "Times required",world.times_required)
		# print("####################")
	...
	def observation(self, agent:Agent, world:World) -> arr:
		...
		# print("Agent", agent.id, "top two goals", top_two_indices, "distances",  world.dists[top_two_indices])
		if min_dist < self.min_obs_dist:
			# If the minimum distance is already less than self.min_dist_thresh, use the previous goal.
			# If the minimum distance is already less than self.min_obs_dist, use the previous goal.
			chosen_goal = np.argmin(world.dists)
			agents_goal = self.landmark_poses[chosen_goal]
			# print("agents_goal",agents_goal,"self.landmark_poses",self.landmark_poses)
			if min_dist < self.min_dist_thresh:
			...
			# 	print("ag! ",agent.id,"NEAR GOAL",chosen_goal, "dist",min_dist, "goal_occupied",self.landmark_poses_occupied[chosen_goal],"occupied flags",self.landmark_poses_occupied,"history",self.goal_history)
				...
				if self.landmark_poses_occupied[chosen_goal] == 0.0:
					...
					if np.any(goal_proximity < self.min_dist_thresh):
						...
						# print("Agent! sorted indices", sorted_indices)
						# top_two_indices = sorted_indices[:2]  # Get indices of top two closest distances
						# top_two_distances = world.dists[top_two_indices]  # Get top two closest distances
						# top_two_chosen_goals = top_two_indices  # Indices of top two closest distances correspond to chosen goals
						# print("Agent! top two goals", top_two_indices, "distances", world.dists[top_two_indices] )
						# agents_goal = self.landmark_poses[chosen_goal]
						pass

						# Now you have the top two closest distances and their corresponding chosen goals
					else:
						# self.landmark_poses_occupied[chosen_goal] = 1.0 - min_dist
			...
			if len(unoccupied_goals) > 0:
				...
				# print("ob2", "agent",agent.id,"goal chosen",agents_goal, "occupied flag",goal_occupied)
	...
	def _get_entity_feat_relative(self, agent:Agent, entity:Entity, world:World, fairness_param: np.ndarray) -> arr:
		...
		if 'agent' in entity.name:
			...
			if min_dist < self.min_obs_dist:
				...
				goal_history = self.goal_history[chosen_goal]
				goal_occupied = np.array([self.landmark_poses_occupied[chosen_goal]])
				# print("gr1", entity.id, "chosen_goal",chosen_goal, "agents_goal",goal_pos, "goal_occupied",goal_occupied)
				...
				if len(unoccupied_goals) > 0:
					...
					goal_occupied = np.array([self.landmark_poses_occupied[unoccupied_goals_indices[min_dist_goal]]])
					goal_history = self.goal_history[unoccupied_goals_indices[min_dist_goal]]
					# print("gr2", "entity",entity.id,"goal chosen",goal_pos, "occupied flag",goal_occupied)
			...
			raise ValueError(f'{entity.name} not supported')
		# print("rel_pos",rel_pos, "rel_vel", rel_vel, "rel_goal_pos", rel_goal_pos, "goal_occupied", goal_occupied, "entity_type", entity_type)
		# print("GRAPH_HSTACK",np.hstack([rel_vel, rel_pos, rel_goal_pos,goal_occupied,rel_pos,rel_pos,entity_type]))
