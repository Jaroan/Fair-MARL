...
class Scenario(BaseScenario):
	def make_world(self, args:argparse.Namespace) -> World:
		"""
			...
			• use_vel: bool
				Whether to use velocity in the node_obs or not
		"""
	...
	def info_callback(self, agent:Agent, world:World) -> Tuple:
		...
		agent_info = {
			...
			'Distance_mean': np.mean(world.dist_left_to_goal), 
			'Distance_variance': np.std(world.dist_left_to_goal),
			'Mean_by_variance': np.mean(world.dist_left_to_goal),
			'Dists_traveled': world.dist_left_to_goal[agent.id],
	...
	def _get_entity_feat_global(self, entity:Entity, world:World) -> arr:
		...
		# else:
		return np.hstack([pos, goal_pos, entity_type])
	...
	def _get_entity_feat_relative(self, agent:Agent, entity:Entity, world:World) -> arr:
		...
		# else:
		return np.hstack([rel_pos, rel_goal_pos, entity_type])
